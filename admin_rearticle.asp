...
<div id="contentmember">
	...
	<div>
		...
		<table style="margin-top:40px;">
			<tr><th>发帖人</th><th>回帖标题</th><th>操作</th></tr>
			<%do while not rs.eof%>
			<tr><td><%=rs("G_UserName")%></td><td><strong title="<%=rs("G_Content")%>"><%=rs("G_Title")%></strong></td><td><a href="admin_re_del.asp?ShowId=<%=rs("G_ID")%>">永久删除</a> <a href="admin_re_hy.asp?ShowId=<%=rs("G_ID")%>">还原</a><td></tr>
			<%
					rs.movenext
				loop
			%>
