...
<%	
	...
	sql="select * from G_Friend where G_ToUser='"&request.Cookies("guest")("username")&"' order by G_Date desc"
...
<!--#include file="member_sidebar.asp"-->
    ...
    <div id="contentmember">
        ...
        <div>
        	...
        	<p class="note_min">你一共被<%=friendcount%>个好友添加</p>
            <table>
                ...
                <% 
                    ...
                    if rs.eof then exit for
						...
						tval=rs("G_Val")
							if tval=0 then
								valstring="未<a href='member_friend_val.asp?fromuser="&rs("G_FromUser")&"&touser="&rs("G_ToUser")&"'>验证</a>,<a href='member_friend_del.asp?fromuser="&rs("G_FromUser")&"&touser="&rs("G_ToUser")&"'>拒绝</a>"
							else
								valstring="双方已是好友"
							end if
				%>
                <tr><td><%=rs("G_FromUser")%></td><td><%=rs("G_Date")%></td><td><%=valstring%></td></tr>
                <%
					rs.movenext
					next
				%>
       		...
       		<span id="page">
                    ...
                    <%
							...
							'否则按照普通的分页来处理
								url="page="&i
                    %>
                        <li><a href="member_friend.asp?<%=url%>"><%=i%></a></li>
					<%
                        next
                    %>
                    </ul>
